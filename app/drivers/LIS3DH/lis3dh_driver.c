...
#define SPIM1_MISO_PIN      14 
#define SPIM1_SS0_PIN       7                      /**< SPI Slave Select GPIO pin number. */
#define LIS3DH_INT1_PIN     6 
#define LIS3DH_INT2_PIN     5 

static uint8_t m_tx_data_spi_1[TX_RX_MSG_LENGTH]; /**< SPI master TX buffer. */
static uint8_t m_rx_data_spi_1[TX_RX_MSG_LENGTH]; /**< SPI master RX buffer. */

uint8_t  SPIReadLength, SPIWriteLength;
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
/* Private function prototypes -----------------------------------------------*/
void LIS3DH_spi_init()
{
    ...
    spi_config.SPI_Pin_SS   = SPIM1_SS0_PIN;
    ...
    nrf_gpio_cfg_default(LIS3DH_INT2_PIN);
}
void LIS3DH_spi_uninit()
{
...
bool LIS3DH_ReadReg(u8_t Reg, u8_t* Data)
{
    ...
    /* Write transaction */
    m_tx_data_spi_1[0] = Reg | LIS3DH_READBIT;
//    (spi_master_tx_rx((uint32_t *)NRF_SPI0,transfer_size,SPIMasterBuffer,SPISlaveBuffer));
    spi_master_send_recv(SPI_MASTER_1, m_tx_data_spi_1,2,m_rx_data_spi_1, 2);
    /* Send received value back to the caller */
    *Data = m_rx_data_spi_1[1];
...
u8_t LIS3DH_WriteReg(u8_t WriteAddr, u8_t Data)
{
    ...
    SPIWriteLength = 2;
    SPIReadLength = 0;
    m_tx_data_spi_1[0] = WriteAddr;
    m_tx_data_spi_1[1] = (Data);
    /* Check if we got an ACK or TIMEOUT error */
//    (spi_master_tx_rx((uint32_t *)NRF_SPI0,SPIWriteLength,SPIMasterBuffer,SPISlaveBuffer));
    spi_master_send_recv(SPI_MASTER_1, m_tx_data_spi_1,SPIWriteLength,m_rx_data_spi_1,SPIWriteLength);
...
status_t LIS3DH_GetAccAxesRaw(AxesRaw_t* buff)
{
    ...
    m_tx_data_spi_1[0] = LIS3DH_OUT_X_L| LIS3DH_READBIT | LIS3DH_MSBIT;

    /* Check if we got an ACK or TIMEOUT error */
    spi_master_send_recv(SPI_MASTER_1, m_tx_data_spi_1,transfer_size+1,accReg, transfer_size+1);
    //(spi_master_tx_rx((uint32_t *)NRF_SPI0,transfer_size+1,SPIMasterBuffer,accReg));
...
bool LIS3DH_enableFiFo(void)
{
    ...
    LIS3DH_WriteReg(LIS3DH_CTRL_REG1,
    ...
    //    ----------------------------------------------------------------------------------------
    (LIS3DH_WriteReg( LIS3DH_CTRL_REG5 ,
                            BIT_6 |   /*Enable FIFO*/
    ...
    (LIS3DH_WriteReg(LIS3DH_CTRL_REG6 ,
    ...
    //    ----------------------------------------------------------------------------------------
    (LIS3DH_WriteReg( LIS3DH_CLICK_CFG ,
    ...
    (LIS3DH_WriteReg( LIS3DH_CLICK_THS , 0x03 ));
    ...
    (LIS3DH_WriteReg( LIS3DH_TIME_LIMIT , 0x33 ));

    (LIS3DH_WriteReg( LIS3DH_TIME_LATENCY , 0xFF ));



    /* Enable FIFO */
    (LIS3DH_WriteReg(LIS3DH_FIFO_CTRL_REG,
                           BIT_6 /* FIFO MODE, trigger signal on INT1 */
                          ));

    /* Enable interrupt */
    (LIS3DH_WriteReg(LIS3DH_INT1_CFG,
                           BIT_6 /* FIFO MODE, trigger signal on INT1 */
                          ));
#endif
...
bool LIS3DH_test()
{
    ...
    //(LIS3DH_WriteReg(LIS3DH_CTRL_REG2,0));
    while(1) {
        //(LIS3DH_WriteReg(LIS3DH_CTRL_REG2,ii));
        for(index = 0;index <20; index ++) {
            (LIS3DH_ReadReg(LIS3DH_CTRL_REG1+index,values+index));
        ...
        /*
         for(index = 0;index <6; index ++)
          {
            (LIS3DH_ReadReg(LIS3DH_OUT_X_L+index,acc+index));  
